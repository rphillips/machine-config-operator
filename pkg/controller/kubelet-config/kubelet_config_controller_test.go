...
func newFeatures(name string, enabled, disabled []string, labels map[string]string) *osev1.Features {
	if labels == nil {
		labels = map[string]string{}
	}
...
func (f *fixture) newController() *Controller {
	...
	c := New(templateDir,
		...
		featinformer.Config().V1().Features(),
	...
	for _, c := range f.featLister {
		featinformer.Config().V1().Features().Informer().GetIndexer().Add(c)
	}
