...
const (
	...
	// kubeletCAPath the ca certificates path for the kubelet
...
func restartKubelet() *exec.Cmd {
	return exec.Command("/bin/sh", "-c", "systemctl stop kubelet.service ; systemctl start kubelet.service")
}
...
func (dn *Daemon) Run(stopCh <-chan struct{}, exitCh <-chan error) error {
	...
	glog.Info("Running Kubelet Cert Monitor")
	go dn.runKubeletCertMonitor(dn.stopCh)
...
var kubeAPICertKey = "kube-apiserver-to-kubelet-client-ca"
var kubeAPICertNs = "openshift-kube-apiserver-operator"

func pemToDer(pemBytes []byte) []byte {
	der := []byte{}
	for len(pemBytes) > 0 {
		b, next := pem.Decode(pemBytes)
		if b == nil {
			break
		}
		pemBytes = next
...
func shouldWriteCert(cert []byte) bool {
	decodedCert, _ := pem.Decode(cert)
	if decodedCert == nil {
		glog.Infof("ConfigMap certificate not in PEM format")
		return false
	}
	// Check to see if passed in cert is valid
	configMapCert, err := x509.ParseCertificate(decodedCert.Bytes)
	if err != nil {
		glog.Infof("ConfigMap certificate parse error: %v", err)
		return false
	}
	// Check to see if cert is valid on local filesystem
	filedata, err := ioutil.ReadFile(kubeletCAPath)
	if err != nil {
		glog.Infof("Local certificate error: %v", err)
		return true
	}
	localFileCerts, err := x509.ParseCertificates(pemToDer(filedata))
	if err != nil {
		glog.Infof("Local certificate parse error: %v", err)
		return true
	}
	for _, fcert := range localFileCerts {
		if !strings.Contains(fcert.Subject.String(), "kube-apiserver-to-kubelet-signer") {
			continue
		}
		// Check to see if attributes have changed
		if fcert.SerialNumber != configMapCert.SerialNumber {
			glog.Infof("ConfigMap certificate serial number changed")
			return true
		}
		if fcert.Version != configMapCert.Version {
			glog.Infof("ConfigMap certificate version changed")
			return true
		}
	}
	glog.Infof("ConfigMap certificate not changed. Skipping rotation.")
	return false
}

func (dn *Daemon) rotateCertificate(cert []byte, shouldRestartKubelet bool) {
	if !shouldWriteCert([]byte(cert)) {
		return
	}
	glog.Infof("Rotating %v certificate", kubeletCAPath)
	if err := writeFileAtomicallyWithDefaults(kubeletCAPath, []byte(cert)); err != nil {
		glog.Infof("Error writing %v: %v", kubeletCAPath, err)
		return
	}
	if shouldRestartKubelet {
		restartKubelet()
	}
}

func (dn *Daemon) runKubeletCertMonitor(stopCh <-chan struct{}) {
	bundleName := "ca-bundle.crt"
	wait.Forever(func() {
		// Initial check to see if the cert needs rotated
		runningCert, err := dn.kubeClient.CoreV1().ConfigMaps(kubeAPICertNs).Get(kubeAPICertKey, metav1.GetOptions{})
		if err == nil {
			if cert, ok := runningCert.Data[bundleName]; ok {
				dn.rotateCertificate([]byte(cert), true)
			}
			runningCert = nil
		}

		// Setup watch
		watcher, err := dn.kubeClient.CoreV1().ConfigMaps(kubeAPICertNs).Watch(metav1.ListOptions{
			FieldSelector: fields.OneTermEqualSelector("metadata.name", kubeAPICertKey).String(),
		})
		if err != nil {
			glog.Warningf("Could not start watch on %v: %v", kubeAPICertKey, err)
			return
		}
		defer func() {
			watcher.Stop()
			glog.Infof("Watch stopped on %v", kubeAPICertKey)
		}()

		glog.Infof("Watch started on %v", kubeAPICertKey)
		for {
			select {
			case <-stopCh:
				return
			case event, ok := <-watcher.ResultChan():
				if !ok {
					glog.Infof("Watch channel has been closed")
					return
				}
				switch event.Type {
				case watch.Added, watch.Modified:
					glog.Infof("ConfigMap %v changed", kubeAPICertKey)
					if cm, ok := event.Object.(*corev1.ConfigMap); ok {
						if cert, ok := cm.Data[bundleName]; ok {
							dn.rotateCertificate([]byte(cert), true)
						}
					}
				case watch.Deleted:
					glog.Infof("%v has been deleted", kubeAPICertKey)
					return
				}
			}
		}
